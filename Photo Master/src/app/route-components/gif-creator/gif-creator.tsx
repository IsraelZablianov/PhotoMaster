...
export interface GifCreatorProps {
    title?: string;
...
export default class GifCreator extends React.Component<GifCreatorProps, GifCreatorState> {
    ...
    onFileChanged(file: File): void {
        ...
        var __this = this;
        this.gif.on("start", function () {
            __this.startTime = new Date();
        });
        ...
        this.gif.on("progress", function (p: number) {
               __this.setState({
        ...
        setTimeout(() => {
            if(__this.videoEl) {
                __this.videoEl.onplay = function(){
                    if(timer) {
                        clearInterval(timer);
                    }
                    timer = setInterval(() => {__this.capture()}, __this.state.intervalValue);
                }

                __this.videoEl.onended = function(){
                    if(timer) {
                        clearInterval(timer);
                    }
                    __this.gif.render();
